...
public class Grammar {
    ...
    public void generateApB(int a, int b) {
        GrammarToken A = new Zero();
        for (int i = 0; i < a; i++) {
            A = new Inc(A);
        }
        GrammarToken C = A;
        GrammarToken B = new Zero();
        generateApBfirst(A);
        Expression last = prove.lastElement();
        for (int i = 0; i < b; i++) {
            generateApBeC(A, B, C, last);
            B = new Inc(B);
            C = new Inc(C);
        }
        toProves =new Expression(new Equals(new Add(A, B), C));
    }

    void generateApBeC(GrammarToken A, GrammarToken B, GrammarToken C, Expression last) {
        GrammarToken varA = new Variable("a");
        GrammarToken varB = new Variable("b");
        GrammarToken varC = new Variable("c");
        GrammarToken abc = new Equals(new Add(A, B), C);
        GrammarToken abici = new Equals(new Add(A, new Inc(B)), new Inc(C));
        GrammarToken abC = new Equals(new Add(A, B), varC);
        GrammarToken abiCi = new Equals(new Add(A, new Inc(B)), new Inc(varC));
        Expression lastImpl = new Expression(new Implication(abc, abici));
        Expression anyC = new Expression(new Any(varC, new Implication(abC, abiCi)));
        Expression minusB = new Expression(new Implication(last, anyC));
        Expression minusC = new Expression(new Implication(anyC, lastImpl));

        prove.add(minusB);
        prove.add(anyC);
        prove.add(minusC);
        prove.add(lastImpl);
        prove.add(new Expression(abici));
    }

    void generateApBfirst(GrammarToken A) {
        GrammarToken varA = new Variable("a");
        GrammarToken varB = new Variable("b");
        GrammarToken varC = new Variable("c");
        Expression last = prove.lastElement();
        GrammarToken aBC = new Equals(new Add(A, varB), varC);
        GrammarToken aBiCi = new Equals(new Add(A, new Inc(varB)), new Inc(varC));
        GrammarToken anyC = new Any(varC, new Implication(aBC, aBiCi));
        Expression anyB = new Expression(new Any(varB, anyC));
        Expression line1 = new Expression(new Implication(last, anyB));
        prove.add(new Expression(new Equals(new Add(A, new Zero()), A)));
        prove.add(line1);
        prove.add(anyB);
    }
